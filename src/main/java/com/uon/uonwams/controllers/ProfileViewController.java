...
public class ProfileViewController extends MenuController implements ControllerInterface {
    ...
    public void setup() {
        ...
        Optional<User> lineManagerUser = WAMSApplication.userData.getUsers().stream().filter(user -> ((Integer) user.getUserId()).equals(loginUser.getLineManagerUserId())).findFirst();
